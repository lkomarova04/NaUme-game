...
exports.PostgresQuestionRepository = void 0;
...
class PostgresQuestionRepository {
    ...
    `);
        const categoryNames = Array.from(new Set(game_1.QUESTION_BANK.map((question) => question.category)));
    ...
    `);
        const existingCountResult = await this.pool.query('SELECT COUNT(*)::text AS count FROM questions');
        const existingCount = Number(existingCountResult.rows[0]?.count ?? '0');
        if (existingCount === 0) {
            for (const question of game_1.QUESTION_BANK) {
...
exports.PostgresQuestionRepository = PostgresQuestionRepository;
